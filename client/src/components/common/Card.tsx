      ...
      <button className="  ">
        ...
        <p>
          <span className="text-sm ">
            수준: {option.level} | 인원 : {option.currentCount}/
            {option.maxCount}
          </span>
        </p>
        <p>
          <span className="text-sm ">시간: {option.workTime}</span>
        </p>
      </button>
    </div>
  );
...
export default function Card({
  ...
  const [workWithList, setWorkWithList] = useState<
    Array<WorkWith.LocItem["item"]>
  >([]);
  useEffect(() => {
    getWorkWithList(id).then((res) => {
      setWorkWithList(res);
    });
  }, [id]);
  return (
    <div className="max-w-2xl  mx-auto">
      <div className="bg-white  shadow-md border border-gray-200 rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="p-5 w-80">
          ...
          {workWithList.length > 0 ? (
            workWithList.map((workWith) => (
              <Fragment key={workWith.id}>
                <DetailCard option={workWith}></DetailCard>
              </Fragment>
            ))
          ) : (
            <h1>데이터를 빠르게 조회중...</h1>
